...
Development happens in a **separate private repository**, `enix/kuik-v3`, not in
`enix/kube-image-keeper` — if an agent misbehaves, the blast radius is that copy.
Its `main` branch is the working branch for the whole v3 effort; the official repo
stays reachable as the `upstream` remote, and work merges back there in reviewed
batches (per milestone, or per release cut).
...
The separate repository is the outer containment; these are the inner ones:

- **Branch protection** on `main`: PR required, CI green required, no force-push,
  no direct commits from the agent token.
- **Minimal-scope token** for agents: contents + issues + pull-requests on
  `enix/kuik-v3` only. No org scopes, no workflow-file write.
