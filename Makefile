...
.PHONY: test-short
test-short: ## Run fast tests only (slow suites opt out via testing.Short()).
...
KIND_CLUSTER ?= kube-image-keeper-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kube-image-keeper-builder
	$(CONTAINER_TOOL) buildx use kube-image-keeper-builder
	...
	- $(CONTAINER_TOOL) buildx rm kube-image-keeper-builder
	rm Dockerfile.cross
...
.PHONY: conform
conform: $(CONFORM) ## Lint commit messages with conform (COMMIT_MSG_FILE if set, commits since BASE_BRANCH otherwise).
