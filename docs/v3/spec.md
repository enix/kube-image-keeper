...
deletion rules: by tag, only this cluster's tags, only past retention.
...
- **the second cluster uploads nothing.** A copy starts by `HEAD`ing the manifest **by digest** in the
  ...
  present already means one tag `PUT` of a few kilobytes, absent means a normal copy. The blobs are
  linked in that same repository, so nothing needs a cross-repository blob mount and kuik never has to
  know which cluster copied what
