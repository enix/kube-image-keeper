...
1. **Choose the source and its credentials together, one candidate at a time.** The origin ref goes first, then — if it does not answer — each `ImageAlternative` entry covering that image, tried in the order the CR declares them and skipping the ones marked `unavailable: true`. For each candidate in turn, resolve *its own* credentials before doing anything else — the origin has no `auth` field to consult, so its order is the pod's `imagePullSecrets` *if* the controller has been granted read access in that namespace → the registry's `perPrefixFallbackAuth` (longest matching prefix) → anonymous; an `ImageAlternative` entry inserts its own explicit `auth`, if set, ahead of that same order — then `HEAD` the candidate with those credentials. The first candidate whose `HEAD` answers is retained, along with the credentials that made it answer, and the loop stops there.
