...
- **Characters illegal in a repository path are normalized** deterministically: the hostname is lowercased, a port `:` becomes `_` (`localhost:5000/foo` → `localhost_5000/foo`), short forms are expanded (`nginx` → `docker.io/library/nginx`).
...
- An entry is skipped only when its repository is listed in `status.repositories` **and** the ref is found present at the destination. No inventory entry means the ref is enqueued straight away, and an inventoried repository says nothing about a tag nobody has pushed into it yet.
...
1. **Choose the source and its credentials together, one candidate at a time.** The origin ref goes first, then — if it does not answer — each `ImageAlternative` entry covering that image, tried in the order the CR declares them and skipping the ones marked [`unavailable: true`](../spec.md#unavailable). An origin that itself matches an entry marked `unavailable: true` goes **last** rather than first, as it does at admission. For each candidate in turn, resolve *its own* credentials before doing anything else — the origin has no `auth` field to consult, so its order is the pod's `imagePullSecrets` *if* the controller has been granted read access in that namespace → the global `fallbackAuth` (most specific entry) → anonymous; an `ImageAlternative` entry inserts its own explicit `auth`, if set, ahead of that same order — then `HEAD` the candidate with those credentials. The first candidate whose `HEAD` answers is retained, along with the credentials that made it answer, and the loop stops there.

   Credentials must be resolved **before** the `HEAD`, per candidate — never a shared or deferred resolution across candidates. Checking availability with the wrong (or no) credentials and only picking credentials afterwards would make every private alternative look unavailable whenever the controller cannot read the matching secret, defeating the fallback in exactly the case it exists for. When it does not answer — registry down, tag deleted, credentials revoked — move to the next candidate. The fallback applies to a first copy and to a re-copy alike, so an image whose origin registry disappeared for good stays re-copyable. Nothing answering at all leaves the image in the desired state and counts it in `status.images.copy.missingSource` (A.9).
...
- **Success** → `ImageCopied` event on the CR; `status.images.copy.available` reflects the new count on the next status write.
...
- **`Warn`** — re-check the source tag's digest on the **source** host's check windows (this is a read of an upstream, so it is paced like any other) and compare it against the copied manifest's digest (a straight equality, since copies are verbatim). Surface the divergence (`CopyOutOfDate`, the `status.images.drifted` count and one `status.driftedImages` entry per ref, holding the upstream digest against the copied one) without touching the copy. Detection without mutation.
  ...
  **This leg turns a ring, and the ring is reported.** A window belongs to the host, but a ring belongs to a **(resource, host)** pair ([Scheduling](../spec.md#one-budget-per-host-one-ring-per-resource)), so a mirror re-reading tags on `quay.io` holds its own ring there — in lexicographic order, one tag per window taken, cursor persisted in `status.checks.registries` so a restart resumes at its successor rather than at the top. Its `cycleDuration` is the delay before a moved upstream tag is noticed, which is the freshness this mirror actually guarantees, and it is measured rather than derived: the ring shares the host's windows with every `ImageMonitor` tracking it. Under `driftPolicy: Ignore` there is no ring and no entry at all.

  Note the asymmetry with Part B: the **source** side is paced and holds a cursor, the **destination** side is not and holds none (B.3). Same object, two registries, two regimes — one is somebody else's quota, the other is the operator's own.
...
A swept entry carries no origin, so it is held, then deleted: its image sits outside the desired state, hence outside the self-check and the copy queue. That state is transient rather than terminal: the moment a pod uses the image again, the desired state computes that same destination tag forward (C.1), the entry leaves `pendingDeletion` and the image is covered again — so an entry rebuilt without its origin costs at most one fall back to the upstream, never a tag left behind.
...
4. **Delete by tag** — using the OCI 1.1 tag-deletion API. Never delete blindly by digest: deleting a manifest by digest removes *every* tag pointing at it, which on a shared repository would destroy other clusters' references, and even on a single cluster would take down `v1` when you meant `latest`. Deleting by tag only is a deliberate, unconditional choice for exactly that reason — not something kuik does only when convenient.

   Not every registry supports tag deletion, and there is no capability to negotiate up front: it is discovered by trying, when a `DELETE` comes back with a status that says it can't (typically `405 Method Not Allowed`). When that happens, the sweep stops attempting further deletes on that destination and the `ImageMirror`'s `Ready` condition flips to `False` with reason `RegistryDeleteUnsupported` (see [`status.md`](../status.md#imagemirror)), instead of retrying forever against a registry that will keep refusing. This and the registry-side GC of untagged artifacts (C.5) are the two prerequisites `cleanup.enabled` adds on top of the mirror's baseline requirements, listed in [Destination registry requirements](../spec.md#destination-registry-requirements) — GC has no equivalent runtime check, since no OCI endpoint answers "will you reclaim untagged content".
...
- **Not everything in the status weighs the same.** One entry cannot be recomputed and losing it does real damage: `repositories` (the registry cannot be asked which repositories we populated, and a repository nobody sweeps is a permanent leak). Three more are kept for continuity, and losing them only costs redundant work, a restarted clock or a gap in reporting: `pendingDeletion` (the tag sweep of C.1 finds the unused tags again, retention restarting from that moment, though the rebuilt entries come back without the `origin` a pod event had supplied, hence held and deleted outside the self-check and the copy queue — the very state the sweep already produces for anything that fell out of use while the controller was down, and which lasts only until a pod uses the image again, [C.2](#c2-wait-out-the-retention)), `failedImageCopies` (rebuilt at the next attempt) and `checks.registries` (the drift ring restarts at the top of its lexicographic order and reports no `cycleDuration` until it laps again). Everything else in the status — the `images` aggregates, the conditions — is derived and recomputed on every reconcile, as are the things that never reach etcd at all: what is in use, what is copied, check verdicts, the copy queue, the window each registry is paced by.
