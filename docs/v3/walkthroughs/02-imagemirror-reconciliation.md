...
- **The tag carries the cluster identity**, not the path. Clusters sharing a registry write into the *same* repository and own only their own tags — this is what makes blob sharing native (A.8) and keeps the CR identical everywhere. Mind the **128-character tag limit**: origin tag + separator + clusterID must fit, so long CI-style tags are truncated with a deterministic hash suffix. That truncation is exactly why nothing may depend on reading a destination tag backwards.
