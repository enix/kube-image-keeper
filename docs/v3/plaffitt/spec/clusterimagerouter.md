...
| `spec.priority` | | Cross-router tie-break only: when two routers match the same image, the one with the **lowest** `priority` wins. Signed integer, default `0`. Does **not** reorder alternatives within a router (that is `alternatives` order). |
| `spec.filter` | | Selects which Pods / namespaces this router applies to (label, annotation and namespace dimensions). Image scoping is done by the `alternatives` prefixes, so the filter's `image` dimension is not used here. See [Resource filtering](../../../concepts/resource-filtering.md). |
...
`spec.priority` only disambiguates **different routers that match the same image**. The router with
the lowest `priority` value wins; ties are broken deterministically by name, and the overlap is
surfaced as a `status` condition so it is not silent.
