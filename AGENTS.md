...
The docs are **versioned** with the [`starlight-versions`](https://starlight-versions.vercel.app) plugin: the current `docs/` tree is served at the site root (labelled `main`), and each archived version (declared in [`website/versions.mjs`](website/versions.mjs)) is served at `/<slug>/`. A version's docs are **not committed in this branch** — they live on a dedicated git branch (e.g. `docs/v2.2`) whose `docs/` tree holds the markdown plus a `version-config.json` sidebar; at build time `sync-docs` runs `git archive <ref> docs` to source them, injecting the per-page `slug:` frontmatter on the fly (so `2.2` is not github-slugified to `22`). The full "add a new version" workflow lives in [`website/README.md`](website/README.md#documentation-versioning).
