...
var _ = Describe("ClusterImageSetMirror Controller", func() {
	...
	Context("When the CISM has namespace filters", func() {
		const (
	...
	Context("When the CISM has pod filters", func() {
		const (
			resourceName = "test-pod-filter"
			image        = "docker.io/library/nginx:latest"
			mirrorImage  = "mirror.example.com/cache/library/nginx:latest"
			namespace    = "ns-pod-filter"
		)
		...
		createCISMAndSeed := func(podFilter kuikv1alpha1.PodFilterDefinition) {
			resource := &kuikv1alpha1.ClusterImageSetMirror{
				ObjectMeta: metav1.ObjectMeta{
					Name:       resourceName,
					Finalizers: []string{imageSetMirrorFinalizer},
		...
		It("drops pods whose labels match an exclude selector", func() {
			createCISMAndSeed(kuikv1alpha1.PodFilterDefinition{
				Labels: kuikv1alpha1.SelectorFilter{Exclude: []string{"cnpg.io/podRole=instance"}},
			})
			createPod("pod-excluded", map[string]string{"cnpg.io/podRole": "instance"}, nil)
		...
		It("keeps pods whose labels don't match an exclude selector", func() {
			...
			createPod("pod-kept", map[string]string{"app": "foo"}, nil)
		...
		It("narrows to pods that match an include label selector", func() {
			...
			createPod("pod-out", map[string]string{"app": "skip-me"}, nil)
		...
		It("supports annotation presence-only includes", func() {
			...
			createPod("pod-no-anno", map[string]string{"app": "foo"}, nil)
