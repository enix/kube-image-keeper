...
func (r *ClusterImageSetAvailabilityReconciler) checkNextForRegistry(ctx context.Context, candidate *monitoringCandidate, registry string, pods []corev1.Pod) (time.Duration, error) {
...
func (r *ClusterImageSetAvailabilityReconciler) resolveCredentials(ctx context.Context, fullRef string, fallbackCredentialSecret *kuikv1alpha1.CredentialSecret, pods []corev1.Pod) ([]corev1.Secret, error) {
	var errs []error
	...
	if fallbackCredentialSecret == nil {
		if len(errs) > 0 {
	...
	key := client.ObjectKey{
		Namespace: fallbackCredentialSecret.Namespace,
		Name:      fallbackCredentialSecret.Name,
	}
